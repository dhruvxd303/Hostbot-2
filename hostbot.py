...
# --- Configuration ---
TOKEN = '7971234808:AAFkLxo4zJdxl0M28ogOjJvOTJY9LUYFzAw' # Replace with your actual token
OWNER_ID = 6317271346 # Replace with your Owner ID
ADMIN_ID = 6317271346 # Replace with your Admin ID (can be same as Owner)
YOUR_USERNAME = '@Dhruv0757' # Replace with your Telegram username (without the @)
UPDATE_CHANNEL = 'https://t.me/hostupdatechannel' # Replace with your update channel link
...
@bot.message_handler(commands=['installmod'])
def install_module(message):
    user_id = message.from_user.id
    admin_id = 6317271346  # your admin Telegram ID
    ...
    bot.reply_to(message, f"📦 Installing module:\n`{module_name}`\n\nPlease wait...", parse_mode="Markdown")

    try:
        command = f"pip install {module_name} --break-system-packages -q"
        result = subprocess.run(command, shell=True, capture_output=True, text=True, timeout=240)

        output = result.stdout.strip() or result.stderr.strip()
        if not output:
            output = "✅ Installed successfully (no output)."
        ...
        bot.send_message(
            message.chat.id,
            f"✅ **Installation Finished for:** `{module_name}`\n\n```\n{output}\n```",
            parse_mode="Markdown"
        )

    except subprocess.TimeoutExpired:
        bot.send_message(message.chat.id, f"⚠️ Installation timeout for `{module_name}`", parse_mode="Markdown")
...
"""
    bot.reply_to(message, sysinfo, parse_mode="HTML")
    
    
    except Exception as e:
        bot.send_message(message.chat.id, f"❌ Error installing `{module_name}`\n\n`{str(e)}`", parse_mode="Markdown")
